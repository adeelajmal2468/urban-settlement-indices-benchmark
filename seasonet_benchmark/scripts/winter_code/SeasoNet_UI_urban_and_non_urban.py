...
def find_otsu_threshold(ui_values):
    """Find threshold using Otsu's method."""
    finite_vals = ui_values[np.isfinite(ui_values)]
...
def compute_ui(swir2,nir):
    ...
    eps = 1e-6
    ratio = (swir2 - nir) / (swir2 + nir + eps) 
    ui = (ratio + 1) * 100
    return ui

def create_overlay(ui, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded ui."""
    overlay = np.zeros((*ui.shape, 4))
    urban_mask = ui > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\ui"
...
def process_sample(i_row):
    ...
    # Compute the ui.
    ui = compute_ui(band_swir_2, band_nir)
    
    ui_smooth = gaussian_filter(ui, sigma=5)
    valid_ui = ui_smooth[np.isfinite(ui) & valid_mask]
    if valid_ui.size < 10:
        return None

    # Use Otsu's method for threshold selection
    T_otsu = find_otsu_threshold(valid_ui)
    ui_classified = (ui_smooth > T_otsu).astype(np.uint8)


# Fine-tuned morphological operations
    structure = np.ones((1, 1))  # Smaller structuring element
    ui_classified = binary_opening(ui_classified, structure=structure).astype(np.uint8)
    ui_classified = binary_closing(ui_classified, structure=structure).astype(np.uint8)
    ui_classified = remove_small_objects(ui_classified.astype(bool), min_size=100)  # Reduced min_size
    ui_classified = remove_small_holes(ui_classified, area_threshold=100)  # Reduced area_threshold
    ui_classified = median_filter(ui_classified, size=2)  # Smaller filter size
    ...
    tp_sample = np.sum((ui_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((ui_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((ui_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((ui_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        ...
        plt.subplot(1, 3, 1)
        plt.imshow(ui, cmap='gray', vmin=ui_min, vmax=ui_max)
        cbar = plt.colorbar(label='ui Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('ui', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(ui, cmap='gray', vmin=ui_min, vmax=ui_max)
        overlay = create_overlay(ui, labels, T_otsu, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title('ui Classification Overlay', fontsize=14, pad=20)
        plt.axis('off')
        ...
        plot_path = os.path.join(output_base, f"ui_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": ui_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="ui Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
        confusion_matrix_path = os.path.join(output_base, "overall_confusion_matrix.png")
        plt.savefig(confusion_matrix_path)
        plt.tight_layout()
        plt.show()

    print("ui Processing complete.")