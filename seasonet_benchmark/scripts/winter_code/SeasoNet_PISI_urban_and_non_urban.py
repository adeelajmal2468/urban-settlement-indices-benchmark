...
def calculate_pisi(blue,nir):
    """Compute pisi based on Chen et al. (2019)."""
    blue_norm = normalize_band(blue)
    nir_norm = normalize_band(nir)
    pisi = (0.8192 * blue_norm) - (0.5735 * nir_norm) + 0.0750 
    return pisi

def create_overlay(pisi, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded pisi."""
    overlay = np.zeros((*pisi.shape, 4))
    urban_mask = pisi > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\pisi"
...
def process_sample(i_row, fixed_threshold=(-0.0558, 0.1462)):
    i, row = i_row
    ...
    pisi = calculate_pisi(band_blue, band_nir)
    pisi_smooth = gaussian_filter(pisi, sigma=7)  # Reduced sigma for less smoothing

    valid_pisi = pisi_smooth[np.isfinite(pisi) & valid_mask]
    if valid_pisi.size < 10:
        return None

    # Use fixed threshold: we use the midpoint of the fixed range for classification.
    T_lower, T_upper = fixed_threshold
    T = (T_lower + T_upper) / 2.0

    pisi_classified = (pisi_smooth > T).astype(np.uint8)


    # Fine-tuned morphological operations
    structure = np.ones((1, 1))  # Smaller structuring element
    pisi_classified = binary_opening(pisi_classified, structure=structure).astype(np.uint8)
    pisi_classified = binary_closing(pisi_classified, structure=structure).astype(np.uint8)
    pisi_classified = remove_small_objects(pisi_classified.astype(bool), min_size=100)  # Reduced min_size
    pisi_classified = remove_small_holes(pisi_classified, area_threshold=100)  # Reduced area_threshold
    pisi_classified = median_filter(pisi_classified, size=2)  # Smaller filter size
    ...
    tp_sample = np.sum((pisi_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((pisi_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((pisi_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((pisi_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        pisi_min = np.nanmin(pisi)
        pisi_max = np.nanmax(pisi)
        plt.figure(figsize=(24, 8))
        plt.subplot(1, 3, 1)
        plt.imshow(pisi, cmap='gray', vmin=pisi_min, vmax=pisi_max)
        cbar = plt.colorbar(label='pisi Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('pisi', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(pisi, cmap='gray', vmin=pisi_min, vmax=pisi_max)
        overlay = create_overlay(pisi, labels, T, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title('pisi Classification Overlay', fontsize=14, pad=20)
        plt.axis('off')
        ...
        plot_path = os.path.join(output_base, f"pisi_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": pisi_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="pisi Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
        confusion_matrix_path = os.path.join(output_base, "overall_confusion_matrix.png")
        plt.savefig(confusion_matrix_path)
        plt.tight_layout()
        plt.show()

    print("pisi Processing complete.")