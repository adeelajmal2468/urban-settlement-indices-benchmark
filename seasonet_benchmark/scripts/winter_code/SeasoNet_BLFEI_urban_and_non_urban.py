...
output_base = r"D:\SeasoNet\Indices Implementation\blfei\1"
...
# -----------------------------------------------------------------------------
# Process every row in the CSV file for BLFEI evaluation with a progress bar
# -----------------------------------------------------------------------------
def process_sample(i_row):
    ...
    valid_blfei = blfei_smooth[np.isfinite(blfei_smooth)]
    if valid_blfei.size == 0:
        print(f"No valid BLFEI values for {folder_name}.")
        return None

    # Use Otsu's method for thresholding (after masking out water)
    try:
        threshold_value = threshold_otsu(blfei_smooth[valid_mask])
    except Exception as e:
    ...
    height, width = blfei.shape
    total_pixels_sample = height * width

    # Evaluate metrics only on pixels within the valid mask
    tp_sample = np.sum((blfei_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((blfei_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((blfei_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((blfei_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 1 == 0:
        ...
        overlay = create_overlay(blfei, labels, threshold_value, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
    ...
    return {
        ...
        "y_pred": blfei_classified.flatten()[valid_mask.flatten()]
    }
