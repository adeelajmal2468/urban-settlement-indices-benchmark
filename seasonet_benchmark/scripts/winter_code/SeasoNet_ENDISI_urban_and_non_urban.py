...
urban_classes = {
    ...
    4: "Road and rail networks", 5: "Port areas", 6: "Airports", 10: "Green urban areas",
    11: "Sport and leisure facilities"
}
nonurban_classes = {
    7: "Mineral extraction sites", 8: "Dump sites", 9: "Construction sites", 12: "Non-irrigated arable land",
    13: "Vineyards", 14: "Fruit trees and berry plantations", 15: "Pastures", 16: "Broad-leaved forest",
    17: "Coniferous forest", 18: "Mixed forest", 19: "Natural grasslands", 20: "Moors and heathland",
    21: "Transitional woodland/shrub", 22: "Beaches, dunes, sands", 23: "Bare rock",
    24: "Sparsely vegetated areas", 25: "Inland marshes", 26: "Peat bogs", 27: "Salt marshes",
    28: "Intertidal flats", 29: "Water courses", 30: "Water bodies", 31: "Coastal lagoons",
    32: "Estuaries", 33: "Sea and ocean"
}

urban_colors = {
    1: [1, 0, 0, 0.6], 2: [0, 1, 0, 0.6], 3: [0, 0, 1, 0.6], 4: [1, 1, 0, 0.6],
    5: [1, 0, 1, 0.6], 6: [0, 1, 1, 0.6], 10: [0.5, 0.5, 0.5, 0.6], 11: [0.5, 0, 0, 0.6]
}

nonurban_colors = {
    7: [0.8, 0.8, 0.8, 0.6], 8: [0.7, 0.7, 0.7, 0.6], 9: [0.6, 0.6, 0.6, 0.6], 12: [0.5, 0.5, 0.0, 0.6],
    13: [0.0, 0.5, 0.0, 0.6], 14: [0.0, 0.6, 0.0, 0.6], 15: [0.0, 0.7, 0.0, 0.6], 16: [0.0, 0.8, 0.0, 0.6],
    17: [0.0, 0.9, 0.0, 0.6], 18: [0.0, 1.0, 0.0, 0.6], 19: [0.1, 0.1, 0.1, 0.6], 20: [0.2, 0.2, 0.2, 0.6],
    21: [0.3, 0.3, 0.3, 0.6], 22: [0.4, 0.4, 0.4, 0.6], 23: [0.5, 0.5, 0.5, 0.6], 24: [0.6, 0.6, 0.6, 0.6],
    25: [0.7, 0.7, 0.7, 0.6], 26: [0.8, 0.8, 0.8, 0.6], 27: [0.9, 0.9, 0.9, 0.6], 28: [1.0, 1.0, 1.0, 0.6],
    29: [0.0, 0.0, 0.5, 0.6], 30: [0.0, 0.0, 1.0, 0.6], 31: [0.0, 0.5, 1.0, 0.6], 32: [0.0, 1.0, 1.0, 0.6],
    33: [0.0, 0.0, 0.8, 0.6]
}
...
def calculate_endisi(blue, green, swir1, swir2):
    """Compute ENDISI based on Chen et al. (2019)."""
    mndwi = calculate_mndwi(green, swir1)
    valid_mask = np.isfinite(blue) & np.isfinite(swir1) & np.isfinite(swir2) & np.isfinite(mndwi)
    if not np.any(valid_mask):
        return np.full_like(blue, np.nan, dtype=np.float32)
    mean_blue = np.nanmean(blue[valid_mask])
    mean_ratio = np.nanmean(swir1[valid_mask] / (swir2[valid_mask] + 1e-6))
    mean_mndwi2 = np.nanmean(mndwi[valid_mask]**2)
    denom = mean_ratio + mean_mndwi2 + 1e-6
    alpha = 2.0 * mean_blue / denom
    factor = (swir1 / (swir2 + 1e-6)) + (mndwi**2)
    numerator = blue - alpha * factor
    denominator = blue + alpha * factor + 1e-6
    endisi = numerator / denominator
    return endisi

def find_otsu_threshold(endisi_values):
    """Find threshold using Otsu's method."""
    finite_vals = endisi_values[np.isfinite(endisi_values)]
    if len(finite_vals) < 50:
        return np.nan
    threshold = threshold_otsu(finite_vals)
    return threshold

def create_overlay(endisi, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded ENDISI."""
    overlay = np.zeros((*endisi.shape, 4))
    urban_mask = endisi > threshold
...
# Set Paths (unchanged)
csv_path = r"D:\SeasoNet\filtered_winter_meta_with_both_urban_nonurban.csv"
base_path = r"D:\SeasoNet"
output_base = r"D:\SeasoNet\Indices Implementation\endisi"
...
def process_sample(i_row):
    ...
    try:
        ...
        band_nir, _ = load_and_preprocess_band(nir_tif_path, 1)
        band_swir1 = resample_to_10m(tif_20m_path, 5, meta_blue)
        band_swir2 = resample_to_10m(tif_20m_path, 6, meta_blue)
    ...
    endisi = calculate_endisi(band_blue, band_green, band_swir1, band_swir2)
    endisi_smooth = gaussian_filter(endisi, sigma=6)  # Reduced sigma for less smoothing

    valid_endisi = endisi_smooth[np.isfinite(endisi_smooth) & valid_mask]
    if valid_endisi.size < 50:
        return None

    # Use Otsu's method for threshold selection
    T_otsu = find_otsu_threshold(valid_endisi)
    endisi_classified = (endisi_smooth > T_otsu).astype(np.uint8)

    # Fine-tuned morphological operations
    structure = np.ones((6, 6))  # Smaller structuring element
    endisi_classified = binary_opening(endisi_classified, structure=structure).astype(np.uint8)
    endisi_classified = binary_closing(endisi_classified, structure=structure).astype(np.uint8)
    endisi_classified = remove_small_objects(endisi_classified.astype(bool), min_size=100)  # Reduced min_size
    endisi_classified = remove_small_holes(endisi_classified, area_threshold=100)  # Reduced area_threshold
    endisi_classified = median_filter(endisi_classified, size=1)  # Smaller filter size
    ...
    tp_sample = np.sum((endisi_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((endisi_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((endisi_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((endisi_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        ...
        plt.subplot(1, 3, 1)
        plt.imshow(endisi, cmap='gray', vmin=-0.5, vmax=0.5)
        cbar = plt.colorbar(label='ENDISI Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('ENDISI (raw)', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(endisi, cmap='gray', vmin=-0.5, vmax=0.5)
        overlay = create_overlay(endisi, labels, T_otsu, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title('ENDISI Classification Overlay', fontsize=14, pad=20)
        plt.axis('off')
        ...
        plot_path = os.path.join(output_base, f"endisi_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": endisi_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="ENDISI Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
        confusion_matrix_path = os.path.join(output_base, "overall_confusion_matrix.png")
        plt.savefig(confusion_matrix_path)
        plt.tight_layout()
        plt.show()

    print("ENDISI Processing complete.")