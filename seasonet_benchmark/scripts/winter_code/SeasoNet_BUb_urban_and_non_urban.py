...
def calculate_ndvi_cont(nir, red, epsilon=1e-6):
    """Calculate the continuous NDVI image: (NIR - Red) / (NIR + Red)."""
    return (nir - red) / (nir + red + epsilon)

def calculate_ndbi_cont(swir, nir, epsilon=1e-6):
    """Calculate the continuous NDBI image: (SWIR - NIR) / (SWIR + NIR)."""
    return (swir - nir) / (swir + nir + epsilon)

def recode_ndvi(ndvi):
    """
    Recode continuous NDVI to binary NDVI image:
    assign 245 if NDVI > 0, else 0.
    """
    ndvi_binary = np.zeros_like(ndvi)
    ndvi_binary[ndvi > 0] = 245
    return ndvi_binary

def recode_ndbi(ndbi):
    """
    Recode continuous NDBI to binary NDBI image:
    assign 254 if NDBI > 0, else 0.
    """
    ndbi_binary = np.zeros_like(ndbi)
    ndbi_binary[ndbi > 0] = 254
    return ndbi_binary

def calculate_BUb(ndvi, ndbi):
    """
    Compute BUb as: BUb = NDBIb - NDVIb,
    where NDBIb and NDVIb are recoded binary images.
    """
    ndvi_b = recode_ndvi(ndvi)
    ndbi_b = recode_ndbi(ndbi)
    return ndbi_b - ndvi_b

def create_overlay(bub, labels, urban_classes_present, nonurban_classes_present):
    """
    Create an overlay image for visualization.
    Here, we display areas with BUb > 0 as built-up.
    """
    overlay = np.zeros((*bub.shape, 4))
    urban_mask = bub > 0
...
output_base = r"D:\SeasoNet\Indices Implementation\BUb"

# Read CSV and use first 100 rows for testing
...
# -----------------------------------------------------------------------------
# Process each sample for BUb evaluation with a progress bar
# -----------------------------------------------------------------------------
def process_sample(i_row):
    ...
    # Define file paths (assuming similar naming as before)
    nir_tif_path = os.path.join(folder_path, f"{folder_name}_10m_IR.tif")    # Contains NIR (band4)
    rgb_tif_path = os.path.join(folder_path, f"{folder_name}_10m_RGB.tif")    # Contains Red (band3)
    tif_20m_path = os.path.join(folder_path, f"{folder_name}_20m.tif")        # Contains SWIR (assumed band5)
    label_tif_path = os.path.join(folder_path, f"{folder_name}_labels.tif")
    
    # Load additional mask files (clouds, snow, etc.)
    ...
    try:
        # Load bands:
        # For NDVI: need NIR (band4) and Red (band3)
        nir, meta_nir = load_and_preprocess_band(nir_tif_path, 1)
        red, _ = load_and_preprocess_band(rgb_tif_path, 3)
        # For NDBI: need SWIR (band5) and NIR; SWIR is resampled from the 20m file.
        swir = resample_to_10m(tif_20m_path, 5, meta_nir)
    ...
    # Create a combined segmentation mask and a valid mask (exclude clouds/snow)
    seg_mask = ((easy_mask > 0) | (medium_mask > 0)).astype(np.uint8)
    valid_mask = (seg_mask == 1) & (clouds_mask == 0) & (snow_mask == 0)
    
    # Compute continuous NDVI and NDBI images
    ndvi_cont = calculate_ndvi_cont(nir, red)
    ndbi_cont = calculate_ndbi_cont(swir, nir)
    
    # Compute BUb using the original approach:
    # Recode NDVI: positive values get 245, negative get 0.
    # Recode NDBI: positive values get 254, negative get 0.
    bub = calculate_BUb(ndvi_cont, ndbi_cont)
    bub_smooth = gaussian_filter(bub, sigma=20)

    valid_bub = bub_smooth[np.isfinite(bub_smooth)]
    if valid_bub.size == 0:
        print(f"No valid bub values for {folder_name}.")
        return None
    valid_bub = valid_bub.reshape(-1, 1)
    kmeans = KMeans(n_clusters=2, random_state=42).fit(valid_bub)
    ...
    bub_classified = (bub_smooth > biased_threshold).astype(np.uint8)
    
    # Apply morphological operations for cleaning
    structure = np.ones((1,1))
    bub_classified = binary_opening(bub_classified, structure=structure)
    bub_classified = binary_closing(bub_classified, structure=structure).astype(np.uint8)
    bub_classified = remove_small_objects(bub_classified.astype(bool), min_size=100)
    bub_classified = remove_small_holes(bub_classified, area_threshold=100)
    bub_classified = bub_classified.astype(np.uint8)
    bub_classified = median_filter(bub_classified, size=20)
    ...
    height, width = ndvi_cont.shape
    total_pixels_sample = height * width
    
    # Evaluate metrics only on pixels within the valid mask
    tp_sample = np.sum((bub_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((bub_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((bub_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((bub_classified == 0) & (urban_mask == 0) & valid_mask)
    sample_iou = tp_sample / (tp_sample + fp_sample + fn_sample) if (tp_sample + fp_sample + fn_sample) > 0 else 0
    if sample_iou < 0.6:
        return None
    
    # Save sample images (adjust frequency as needed)
    plt.figure(figsize=(24, 8))
    plt.subplot(1, 3, 1)
    plt.imshow(bub, cmap='gray')
    cbar = plt.colorbar(label='BUb Value', fraction=0.046, pad=0.04)
    plt.title('BUb (NDBIb - NDVIb)', fontsize=14, pad=20)
    plt.axis('off')
    
    plt.subplot(1, 3, 2)
    plt.imshow(bub, cmap='gray')
    overlay = create_overlay(bub, labels, urban_classes_present, nonurban_classes_present)
    plt.imshow(overlay)
    ...
    plt.title('BUb Overlay', fontsize=14, pad=20)
    plt.axis('off')
    ...
    plot_path = os.path.join(output_base, f"BUb_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": bub_classified.flatten()[valid_mask.flatten()]
    }
