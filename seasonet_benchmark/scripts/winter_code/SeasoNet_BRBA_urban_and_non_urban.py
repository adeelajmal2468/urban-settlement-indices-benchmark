...
def calculate_brba(red, swir1):
    """Compute brba based on Chen et al. (2019)."""
    epsilon = 1e-6

    numerator = red
    denominator = swir1 + epsilon
    brba = numerator / denominator
    return brba

def find_otsu_threshold(brba_values):
    """Find threshold using Otsu's method."""
    finite_vals = brba_values[np.isfinite(brba_values)]
    if len(finite_vals) < 50:
        return np.nan
    threshold = threshold_otsu(finite_vals)
    return threshold

def create_overlay(brba, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded brba."""
    overlay = np.zeros((*brba.shape, 4))
    urban_mask = brba > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\brba"
...
def process_sample(i_row):
    ...
    brba = calculate_brba(band_red, band_swir_1)
    brba_smooth = gaussian_filter(brba, sigma=7)  # Reduced sigma for less smoothing

    valid_brba = brba_smooth[np.isfinite(brba) & valid_mask]
    if valid_brba.size < 50:
        return None

    # Use Otsu's method for threshold selection
    T_otsu = find_otsu_threshold(valid_brba)
    brba_classified = (brba_smooth > T_otsu).astype(np.uint8)

    # Fine-tuned morphological operations
    structure = np.ones((1, 1))  # Smaller structuring element
    brba_classified = binary_opening(brba_classified, structure=structure).astype(np.uint8)
    brba_classified = binary_closing(brba_classified, structure=structure).astype(np.uint8)
    brba_classified = remove_small_objects(brba_classified.astype(bool), min_size=100)  # Reduced min_size
    brba_classified = remove_small_holes(brba_classified, area_threshold=100)  # Reduced area_threshold
    brba_classified = median_filter(brba_classified, size=1)  # Smaller filter size
    ...
    tp_sample = np.sum((brba_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((brba_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((brba_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((brba_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        brba_min = np.nanmin(brba)
        brba_max = np.nanmax(brba)
        plt.figure(figsize=(24, 8))
        plt.subplot(1, 3, 1)
        plt.imshow(brba, cmap='gray', vmin=brba_min, vmax=brba_max)
        cbar = plt.colorbar(label='brba Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('brba', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(brba, cmap='gray', vmin=brba_min, vmax=brba_max)
        overlay = create_overlay(brba, labels, T_otsu, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title('brba Classification Overlay', fontsize=14, pad=20)
        plt.axis('off')
        ...
        plot_path = os.path.join(output_base, f"brba_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": brba_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="brba Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
        confusion_matrix_path = os.path.join(output_base, "overall_confusion_matrix.png")
        plt.savefig(confusion_matrix_path)
        plt.tight_layout()
        plt.show()

    print("brba Processing complete.")