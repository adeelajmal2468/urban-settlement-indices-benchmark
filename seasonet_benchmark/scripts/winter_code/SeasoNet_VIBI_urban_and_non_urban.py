...
nonurban_classes = {
    ...
    13: "Vineyards",
    14: "Frvibit trees and berry plantations",
    15: "Pastures",
...
def find_otsu_threshold(vibi_values):
    """Find threshold using Otsu's method."""
    finite_vals = vibi_values[np.isfinite(vibi_values)]
...
def compute_vibi(ndvi, ndbi):
    
    eps = 1e-6
    vibi = ndvi / (ndvi + ndbi + eps)
    return vibi

def create_overlay(vibi, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded vibi."""
    overlay = np.zeros((*vibi.shape, 4))
    urban_mask = vibi > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\vibi"
...
def process_sample(i_row):
    ...
    ndvi = compute_ndvi(band_nir, band_red)
    
    ndbi = compute_ndbi(band_swir_1, band_nir)
    ...
    # Compute the vibi.
    vibi = compute_vibi(ndvi, ndbi)
    vibi_smooth = gaussian_filter(vibi, sigma=27)
    valid_vibi = vibi_smooth[np.isfinite(vibi_smooth)]
    if valid_vibi.size == 0:
        print(f"No valid cbci values for {folder_name}.")
        return None

    valid_vibi = valid_vibi.reshape(-1, 1)
    kmeans = KMeans(n_clusters=2, random_state=42).fit(valid_vibi)
    ...
    #T_otsu = find_otsu_threshold(valid_vibi)
    #T_otsu = 0
    vibi_classified = (vibi_smooth > biased_threshold).astype(np.uint8)


# Fine-tuned morphological operations
    structure = np.ones((1, 1))  # Smaller structuring element
    vibi_classified = binary_opening(vibi_classified, structure=structure).astype(np.uint8)
    vibi_classified = binary_closing(vibi_classified, structure=structure).astype(np.uint8)
    vibi_classified = remove_small_objects(vibi_classified.astype(bool), min_size=100)  # Reduced min_size
    vibi_classified = remove_small_holes(vibi_classified, area_threshold=100)  # Reduced area_threshold
    vibi_classified = median_filter(vibi_classified, size=1)  # Smaller filter size
    ...
    tp_sample = np.sum((vibi_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((vibi_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((vibi_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((vibi_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        # vibi_min = np.nanmin(vibi)
        # vibi_max = np.nanmax(vibi)
        plt.figure(figsize=(24, 8))
        plt.subplot(1, 3, 1)
        plt.imshow(vibi, cmap='gray', vmin = -1, vmax = 1)
        cbar = plt.colorbar(label='vibi Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('vibi', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(vibi, cmap='gray', vmin = -1, vmax = 1)
        overlay = create_overlay(vibi, labels, biased_threshold, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title('vibi Classification Overlay', fontsize=14, pad=20)
        plt.axis('off')
        ...
        plot_path = os.path.join(output_base, f"vibi_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": vibi_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="vibi Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
        confusion_matrix_path = os.path.join(output_base, "overall_confusion_matrix.png")
        plt.savefig(confusion_matrix_path)
        plt.tight_layout()
        plt.show()

    print("vibi Processing complete.")