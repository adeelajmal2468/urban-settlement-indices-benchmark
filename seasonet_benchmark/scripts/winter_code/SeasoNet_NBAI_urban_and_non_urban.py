...
def calculate_mndwi(green,swir1):
    epsilon = 1e-6
    n = green - swir1
    d = green + swir1 + epsilon
    return n /d

def calculate_nbai(swir2, swir1, green):
    """Compute nbai based on Chen et al. (2019)."""
    epsilon = 1e-6
    swir2_norm = normalize_band(swir2)
    swir1_norm = normalize_band(swir1)
    green_norm = normalize_band(green)
    numerator = ((green_norm * swir2_norm) - swir1_norm)
    denominator = ((green_norm * swir2_norm) + swir1_norm + epsilon)
    nbai = numerator / denominator
    return nbai

def find_otsu_threshold(nbai_values):
    """Find threshold using Otsu's method."""
    finite_vals = nbai_values[np.isfinite(nbai_values)]
    if len(finite_vals) < 50:
        return np.nan
    threshold = threshold_otsu(finite_vals)
    return threshold

def create_overlay(nbai, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded nbai."""
    overlay = np.zeros((*nbai.shape, 4))
    urban_mask = nbai > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\nbai"
...
def process_sample(i_row):
    ...
    mndwi = calculate_mndwi(band_green,band_swir_1)
    water_threshold = 0.3
    water_mask = mndwi > water_threshold
    valid_mask &= ~water_mask
    nbai = calculate_nbai(band_swir_2, band_swir_1, band_green)
    nbai_smooth = gaussian_filter(nbai, sigma=6)  # Reduced sigma for less smoothing

    valid_nbai = nbai_smooth[np.isfinite(nbai) & valid_mask]
    if valid_nbai.size < 50:
        return None

    # Use Otsu's method for threshold selection
    T_otsu = find_otsu_threshold(valid_nbai)
    nbai_classified = (nbai_smooth > T_otsu).astype(np.uint8)

    # Fine-tuned morphological operations
    structure = np.ones((1, 1))  # Smaller structuring element
    nbai_classified = binary_opening(nbai_classified, structure=structure).astype(np.uint8)
    nbai_classified = binary_closing(nbai_classified, structure=structure).astype(np.uint8)
    nbai_classified = remove_small_objects(nbai_classified.astype(bool), min_size=100)  # Reduced min_size
    nbai_classified = remove_small_holes(nbai_classified, area_threshold=100)  # Reduced area_threshold
    nbai_classified = median_filter(nbai_classified, size=1)  # Smaller filter size
    ...
    tp_sample = np.sum((nbai_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((nbai_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((nbai_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((nbai_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        nbai_min = np.nanmin(nbai)
        nbai_max = np.nanmax(nbai)
        plt.figure(figsize=(24, 8))
        plt.subplot(1, 3, 1)
        plt.imshow(nbai, cmap='gray', vmin=nbai_min, vmax=nbai_max)
        cbar = plt.colorbar(label='nbai Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('nbai', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(nbai, cmap='gray', vmin=nbai_min, vmax=nbai_max)
        overlay = create_overlay(nbai, labels, T_otsu, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title('nbai Classification Overlay', fontsize=14, pad=20)
        plt.axis('off')
        ...
        plot_path = os.path.join(output_base, f"nbai_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": nbai_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="nbai Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
        confusion_matrix_path = os.path.join(output_base, "overall_confusion_matrix.png")
        plt.savefig(confusion_matrix_path)
        plt.tight_layout()
        plt.show()

    print("nbai Processing complete.")