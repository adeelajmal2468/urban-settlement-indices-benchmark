...
def find_otsu_threshold(bci_values):
    """Find threshold using Otsu's method."""
    finite_vals = bci_values[np.isfinite(bci_values)]
...
def compute_BCI(TC_brightness, TC_greenness, TC_wetness):
    """
    Compute the Biophysical Composition Index (BCI) as:
      BCI = ((H + L)/2 - V) / ((H + L)/2 + V)
    where H = normalized TC_brightness, V = normalized TC_greenness,
          L = normalized TC_wetness.
    Returns BCI, H_norm, V_norm, L_norm.
    """
    H_norm = normalize_band(TC_brightness)
    V_norm = normalize_band(TC_greenness)
    L_norm = normalize_band(TC_wetness)
    eps = 1e-6
    numerator = H_norm + L_norm - 2* V_norm
    denominator = H_norm + L_norm + 2* V_norm + eps
    BCI = numerator / denominator
    BCI_norm = normalize_band(BCI)
    return BCI_norm

def create_overlay(bci, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded bci."""
    overlay = np.zeros((*bci.shape, 4))
    urban_mask = bci > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\bci"

df_all = pd.read_csv(csv_path)
#df_all = df_all.head(100)

# Coefficient arrays for Sentinel-2 Tasseled Cap transformation (example values from Nedkov 2017)
...
coeff_greenness  = np.array([-0.2266,-0.2818, -0.3020, -0.4283, -0.2959, 0.1602, 0.3127, 0.3138, 0.4261, 0.1454, -0.1341, -0.2538], dtype=np.float32)
coeff_wetness    = np.array([0.1825, 0.1763, 0.1615, 0.0486, 0.0170, 0.0223, 0.0219, -0.0755, -0.0910, -0.1369, -0.7701, -0.5293], dtype=np.float32)
...
def process_sample(i_row):
    ...
    TC_brightness, TC_greenness, TC_wetness = compute_tasseled_cap(stack,
                                                                     coeff_brightness,
                                                                     coeff_greenness,
                                                                     coeff_wetness)
    ...
    # Compute the BCI.
    bci = compute_BCI(TC_brightness, TC_greenness, TC_wetness)
    
    bci_smooth = gaussian_filter(bci, sigma=6)
    valid_bci = bci_smooth[np.isfinite(bci) & valid_mask]
    if valid_bci.size < 10:
        return None

    # Use Otsu's method for threshold selection
    T_otsu = find_otsu_threshold(valid_bci)
    bci_classified = (bci_smooth > T_otsu).astype(np.uint8)

# Fine-tuned morphological operations
    structure = np.ones((1, 1))  # Smaller structuring element
    bci_classified = binary_opening(bci_classified, structure=structure).astype(np.uint8)
    bci_classified = binary_closing(bci_classified, structure=structure).astype(np.uint8)
    bci_classified = remove_small_objects(bci_classified.astype(bool), min_size=100)  # Reduced min_size
    bci_classified = remove_small_holes(bci_classified, area_threshold=100)  # Reduced area_threshold
    bci_classified = median_filter(bci_classified, size=1)  # Smaller filter size
    ...
    tp_sample = np.sum((bci_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((bci_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((bci_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((bci_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        bci_min = np.nanmin(bci)
        bci_max = np.nanmax(bci)
        plt.figure(figsize=(24, 8))
        plt.subplot(1, 3, 1)
        plt.imshow(bci, cmap='gray', vmin=bci_min, vmax=bci_max)
        cbar = plt.colorbar(label='bci Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('bci', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(bci, cmap='gray', vmin=bci_min, vmax=bci_max)
        overlay = create_overlay(bci, labels, T_otsu, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title('bci Classification Overlay', fontsize=14, pad=20)
        plt.axis('off')
        ...
        plot_path = os.path.join(output_base, f"bci_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": bci_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="bci Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
        confusion_matrix_path = os.path.join(output_base, "overall_confusion_matrix.png")
        plt.savefig(confusion_matrix_path)
        plt.tight_layout()
        plt.show()

    print("bci Processing complete.")