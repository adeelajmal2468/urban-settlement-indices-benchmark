...
output_base = r"D:\SeasoNet\Indices Implementation\SwiRed"
...
def process_sample(i_row):
    ...
    valid_SwiRed = SwiRed_smooth[np.isfinite(SwiRed) & valid_mask]
...
# Fine-tuned morphological operations
    ...
    SwiRed_classified = remove_small_objects(SwiRed_classified.astype(bool), min_size=100)  # Reduced min_size
    SwiRed_classified = remove_small_holes(SwiRed_classified, area_threshold=100)  # Reduced area_threshold
    SwiRed_classified = median_filter(SwiRed_classified, size=1)  # Smaller filter size
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="SwiRed Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
