...
def calculate_mbai(nir, green, swir):
    """Compute mbai based on Chen et al. (2019)."""
    epsilon = 1e-6
    numerator = (nir + 1.57 * green  + 2.4* swir)
    denominator = (1 + nir + epsilon)
    mbai = numerator / denominator
    return mbai

def find_otsu_threshold(mbai_values):
    """Find threshold using Otsu's method."""
    finite_vals = mbai_values[np.isfinite(mbai_values)]
    if len(finite_vals) < 50:
        return np.nan
    threshold = threshold_otsu(finite_vals)
    return threshold

def create_overlay(mbai, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded mbai."""
    overlay = np.zeros((*mbai.shape, 4))
    urban_mask = mbai > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\mbai"
...
def process_sample(i_row):
    ...
    mbai = calculate_mbai(band_nir, band_green, band_swir)
    mbai_smooth = gaussian_filter(mbai, sigma=6)  # Reduced sigma for less smoothing

    valid_mbai = mbai_smooth[np.isfinite(mbai_smooth) & valid_mask]
    if valid_mbai.size < 50:
        return None

    # Use Otsu's method for threshold selection
    T_otsu = find_otsu_threshold(valid_mbai)
    mbai_classified = (mbai_smooth > T_otsu).astype(np.uint8)

    # Fine-tuned morphological operations
    structure = np.ones((6, 6))  # Smaller structuring element
    mbai_classified = binary_opening(mbai_classified, structure=structure).astype(np.uint8)
    mbai_classified = binary_closing(mbai_classified, structure=structure).astype(np.uint8)
    mbai_classified = remove_small_objects(mbai_classified.astype(bool), min_size=100)  # Reduced min_size
    mbai_classified = remove_small_holes(mbai_classified, area_threshold=100)  # Reduced area_threshold
    mbai_classified = median_filter(mbai_classified, size=1)  # Smaller filter size
    ...
    tp_sample = np.sum((mbai_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((mbai_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((mbai_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((mbai_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        mbai_min = np.nanmin(mbai)
        mbai_max = np.nanmax(mbai)
        plt.figure(figsize=(24, 8))
        plt.subplot(1, 3, 1)
        plt.imshow(mbai, cmap='gray', vmin=mbai_min, vmax=mbai_max)
        cbar = plt.colorbar(label='mbai Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('mbai', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(mbai, cmap='gray', vmin=mbai_min, vmax=mbai_max)
        overlay = create_overlay(mbai, labels, T_otsu, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title('mbai Classification Overlay', fontsize=14, pad=20)
        plt.axis('off')
        ...
        plot_path = os.path.join(output_base, f"mbai_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": mbai_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="mbai Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
        confusion_matrix_path = os.path.join(output_base, "overall_confusion_matrix.png")
        plt.savefig(confusion_matrix_path)
        plt.tight_layout()
        plt.show()

    print("mbai Processing complete.")