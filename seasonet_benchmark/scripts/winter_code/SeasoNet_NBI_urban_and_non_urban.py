...
def calculate_nbi(red, swir1, nir):
    """Compute nbi based on Chen et al. (2019)."""
    epsilon = 1e-6

    numerator = (red * swir1)
    denominator = nir + epsilon
    nbi = numerator / denominator
    return nbi

def find_otsu_threshold(nbi_values):
    """Find threshold using Otsu's method."""
    finite_vals = nbi_values[np.isfinite(nbi_values)]
    if len(finite_vals) < 50:
        return np.nan
    threshold = threshold_otsu(finite_vals)
    return threshold

def create_overlay(nbi, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded nbi."""
    overlay = np.zeros((*nbi.shape, 4))
    urban_mask = nbi > threshold
...
def process_sample(i_row):
    ...
    nbi = calculate_nbi(band_red, band_swir_1, band_nir)
    nbi_smooth = gaussian_filter(nbi, sigma=6)  # Reduced sigma for less smoothing

    valid_nbi = nbi_smooth[np.isfinite(nbi) & valid_mask]
    if valid_nbi.size < 10:
        return None

    # Use Otsu's method for threshold selection
    T_otsu = find_otsu_threshold(valid_nbi)
    nbi_classified = (nbi_smooth > T_otsu).astype(np.uint8)

    # Fine-tuned morphological operations
    structure = np.ones((4, 4))  # Smaller structuring element
    nbi_classified = binary_opening(nbi_classified, structure=structure).astype(np.uint8)
    nbi_classified = binary_closing(nbi_classified, structure=structure).astype(np.uint8)
    nbi_classified = remove_small_objects(nbi_classified.astype(bool), min_size=100)  # Reduced min_size
    nbi_classified = remove_small_holes(nbi_classified, area_threshold=100)  # Reduced area_threshold
    nbi_classified = median_filter(nbi_classified, size=1)  # Smaller filter size
    ...
    tp_sample = np.sum((nbi_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((nbi_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((nbi_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((nbi_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        nbi_min = np.nanmin(nbi)
        nbi_max = np.nanmax(nbi)
        plt.figure(figsize=(24, 8))
        plt.subplot(1, 3, 1)
        plt.imshow(nbi, cmap='gray', vmin=nbi_min, vmax=nbi_max)
        cbar = plt.colorbar(label='nbi Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('nbi', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(nbi, cmap='gray', vmin=nbi_min, vmax=nbi_max)
        overlay = create_overlay(nbi, labels, T_otsu, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title('nbi Classification Overlay', fontsize=14, pad=20)
        plt.axis('off')
        ...
        plot_path = os.path.join(output_base, f"nbi_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": nbi_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="nbi Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
        confusion_matrix_path = os.path.join(output_base, "overall_confusion_matrix.png")
        plt.savefig(confusion_matrix_path)
        plt.tight_layout()
        plt.show()

    print("nbi Processing complete.")