...
nonurban_classes = {
    ...
    13: "Vineyards",
    14: "Frwe_ndbit trees and berry plantations",
    15: "Pastures",
...
def find_otsu_threshold(we_ndbi_values):
    """Find threshold using Otsu's method."""
    finite_vals = we_ndbi_values[np.isfinite(we_ndbi_values)]
    if len(finite_vals) < 50:
        return np.nan
    threshold = threshold_otsu(finite_vals)
    return threshold

def compute_ndbi_gf1(red, green):
    
    eps = 1e-6
    ndbi_gf1 = (red - green) / (red + green + eps)
    return ndbi_gf1
...
def create_overlay(we_ndbi, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded we_ndbi."""
    overlay = np.zeros((*we_ndbi.shape, 4))
    urban_mask = we_ndbi > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\we-ndbi"
...
def process_sample(i_row):
    ...
    ndbi_gf1 = compute_ndbi_gf1(band_red, band_green)
   # Step 3: Water-Erasing Step (Compute W and apply WE-NDBI)
    water_ndbi_values = ndbi_gf1[water_mask & np.isfinite(ndbi_gf1)]
    if water_ndbi_values.size > 0:
        W = np.min(water_ndbi_values)  # Minimum NDBI_GF1 in water areas
    else:
        W = np.nan  # Handle case with no water pixels

    # Compute WE-NDBI: set to 0 where NDBI_GF1 > W
    if np.isfinite(W):
        we_ndbi = np.where(ndbi_gf1 <= W, ndbi_gf1, 0)
    else:
        we_ndbi = ndbi_gf1  # If no water, WE-NDBI = NDBI_GF1
    we_ndbi_smooth = gaussian_filter(we_ndbi, sigma=6)
    # valid_we_ndbi = we_ndbi_smooth[np.isfinite(we_ndbi_smooth)]
    # if valid_we_ndbi.size == 0:
    #     print(f"No valid cbci values for {folder_name}.")
    #     return None
    # valid_we_ndbi = valid_we_ndbi.reshape(-1, 1)
    # kmeans = KMeans(n_clusters=2, random_state=42).fit(valid_we_ndbi)
    ...
    valid_we_ndbi = we_ndbi_smooth[np.isfinite(we_ndbi) & valid_mask]
    if valid_we_ndbi.size < 10:
        return None
    # # Use Otsu's method for threshold selection
    T_otsu = find_otsu_threshold(valid_we_ndbi)
    # T_otsu = 0
    we_ndbi_classified = (we_ndbi_smooth > T_otsu).astype(np.uint8)


# Fine-tuned morphological operations
    structure = np.ones((1, 1))  # Smaller structuring element
    we_ndbi_classified = binary_opening(we_ndbi_classified, structure=structure).astype(np.uint8)
    we_ndbi_classified = binary_closing(we_ndbi_classified, structure=structure).astype(np.uint8)
    we_ndbi_classified = remove_small_objects(we_ndbi_classified.astype(bool), min_size=100)  # Reduced min_size
    we_ndbi_classified = remove_small_holes(we_ndbi_classified, area_threshold=100)  # Reduced area_threshold
    we_ndbi_classified = median_filter(we_ndbi_classified, size=2)  # Smaller filter size
    ...
    tp_sample = np.sum((we_ndbi_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((we_ndbi_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((we_ndbi_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((we_ndbi_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        we_ndbi_min = np.nanmin(we_ndbi)
        we_ndbi_max = np.nanmax(we_ndbi)
        plt.figure(figsize=(24, 8))
        plt.subplot(1, 3, 1)
        plt.imshow(we_ndbi, cmap='gray', vmin =we_ndbi_min, vmax = we_ndbi_max)
        cbar = plt.colorbar(label='we_ndbi Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('we_ndbi', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(we_ndbi, cmap='gray', vmin = we_ndbi_min, vmax = we_ndbi_max)
        overlay = create_overlay(we_ndbi, labels, T_otsu, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title('we_ndbi Classification Overlay', fontsize=14, pad=20)
        plt.axis('off')
        ...
        plot_path = os.path.join(output_base, f"we_ndbi_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": we_ndbi_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="we_ndbi Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
        confusion_matrix_path = os.path.join(output_base, "overall_confusion_matrix.png")
        plt.savefig(confusion_matrix_path)
        plt.tight_layout()
        plt.show()

    print("we_ndbi Processing complete.")