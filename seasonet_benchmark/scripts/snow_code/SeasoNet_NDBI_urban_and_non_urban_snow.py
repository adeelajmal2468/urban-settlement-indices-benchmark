...
def create_overlay(ndbi_values, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded NDBI."""
    overlay = np.zeros((*ndbi_values.shape, 4))
    urban_mask = ndbi_values > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\snow_results\ndbi"
...
def process_sample(i_row):
    ...
    nir_tif_path = os.path.join(folder_path, f"{folder_name}_10m_IR.tif")    # Band 8 NIR
    tif_20m_path = os.path.join(folder_path, f"{folder_name}_20m.tif")      # includes SWIR1
    ...
    ndbi_smooth = gaussian_filter(ndbi, sigma=6)

    valid_ndbi = ndbi_smooth[np.isfinite(ndbi_smooth) & valid_mask]
    if valid_ndbi.size < 10:
        return None

    # Otsu threshold on NDBI
    T_otsu = find_otsu_threshold(valid_ndbi)

    # If threshold is not finite skip this sample
    if not np.isfinite(T_otsu):
        return None

    ndbi_classified = (ndbi_smooth > T_otsu).astype(np.uint8)

    # Morphological cleaning
    structure = np.ones((1, 1))
    ndbi_classified = binary_opening(ndbi_classified, structure=structure).astype(np.uint8)
    ndbi_classified = binary_closing(ndbi_classified, structure=structure).astype(np.uint8)
    ndbi_classified = remove_small_objects(ndbi_classified.astype(bool), min_size=100)
    ndbi_classified = remove_small_holes(ndbi_classified, area_threshold=100)
    ndbi_classified = median_filter(ndbi_classified.astype(np.uint8), size=2)
    ndbi_classified = (ndbi_classified > 0).astype(np.uint8)
    ...
    # Diagnostic plots every 50 samples
    if i % 50 == 0:
        ...
        # Raw NDBI
        plt.subplot(1, 3, 1)
        plt.imshow(ndbi, cmap="gray", vmin=ndbi_min, vmax=ndbi_max)
        cbar = plt.colorbar(label="NDBI Value", fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title("NDBI", fontsize=14, pad=20)
        plt.axis("off")

        # NDBI classification overlay
        plt.subplot(1, 3, 2)
        plt.imshow(ndbi, cmap="gray", vmin=ndbi_min, vmax=ndbi_max)
        overlay = create_overlay(ndbi_smooth, labels, T_otsu, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title("NDBI Classification Overlay", fontsize=14, pad=20)
        plt.axis("off")
    ...
    return {
        ...
        "y_pred": ndbi_classified.flatten()[valid_mask.flatten()],
    }
...
if __name__ == "__main__":
    ...
    if total_pixels_overall > 0:
        ...
        overall_iou = (
            ...
            if (overall_tp + overall_fp + overall_fn)
            else 0
        )
    else:
        overall_accuracy = 0
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(
            all_y_true,
            all_y_pred,
            title="NDBI Overall Confusion Matrix",
    ...
    print("NDBI processing complete.")
