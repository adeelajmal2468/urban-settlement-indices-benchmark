...
nonurban_classes = {
    ...
    13: "Vineyards",
    14: "Frvgnir_bit trees and berry plantations",
    15: "Pastures",
...
def find_otsu_threshold(vgnir_bi_values):
    """Find threshold using Otsu's method."""
    finite_vals = vgnir_bi_values[np.isfinite(vgnir_bi_values)]
...
def compute_vgnir_bi(green, nir):
    
    eps = 1e-6
    vgnir_bi = (green - nir) / (green + nir + eps)
    return vgnir_bi
...
def create_overlay(vgnir_bi, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded vgnir_bi."""
    overlay = np.zeros((*vgnir_bi.shape, 4))
    urban_mask = vgnir_bi > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\snow_results\vgnir-bi"
...
def process_sample(i_row):
    ...
    # Compute the vgnir_bi.
    vgnir_bi = compute_vgnir_bi(band_green, band_nir)
    vgnir_bi_smooth = gaussian_filter(vgnir_bi, sigma=6)
    # valid_vgnir_bi = vgnir_bi_smooth[np.isfinite(vgnir_bi_smooth)]
    # if valid_vgnir_bi.size == 0:
    #     print(f"No valid cbci values for {folder_name}.")
    #     return None
    # valid_vgnir_bi = valid_vgnir_bi.reshape(-1, 1)
    # kmeans = KMeans(n_clusters=2, random_state=42).fit(valid_vgnir_bi)
    ...
    valid_vgnir_bi = vgnir_bi_smooth[np.isfinite(vgnir_bi) & valid_mask]
    if valid_vgnir_bi.size < 10:
        return None
    # # Use Otsu's method for threshold selection
    T_otsu = find_otsu_threshold(valid_vgnir_bi)
    # T_otsu = 0
    vgnir_bi_classified = (vgnir_bi_smooth > T_otsu).astype(np.uint8)


# Fine-tuned morphological operations
    structure = np.ones((1, 1))  # Smaller structuring element
    vgnir_bi_classified = binary_opening(vgnir_bi_classified, structure=structure).astype(np.uint8)
    vgnir_bi_classified = binary_closing(vgnir_bi_classified, structure=structure).astype(np.uint8)
    vgnir_bi_classified = remove_small_objects(vgnir_bi_classified.astype(bool), min_size=100)  # Reduced min_size
    vgnir_bi_classified = remove_small_holes(vgnir_bi_classified, area_threshold=100)  # Reduced area_threshold
    vgnir_bi_classified = median_filter(vgnir_bi_classified, size=1)  # Smaller filter size
    ...
    tp_sample = np.sum((vgnir_bi_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((vgnir_bi_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((vgnir_bi_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((vgnir_bi_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        vgnir_bi_min = np.nanmin(vgnir_bi)
        vgnir_bi_max = np.nanmax(vgnir_bi)
        plt.figure(figsize=(24, 8))
        plt.subplot(1, 3, 1)
        plt.imshow(vgnir_bi, cmap='gray', vmin =vgnir_bi_min, vmax = vgnir_bi_max)
        cbar = plt.colorbar(label='vgnir_bi Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('vgnir_bi', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(vgnir_bi, cmap='gray', vmin = vgnir_bi_min, vmax = vgnir_bi_max)
        overlay = create_overlay(vgnir_bi, labels, T_otsu, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title('vgnir_bi Classification Overlay', fontsize=14, pad=20)
        plt.axis('off')
        ...
        plot_path = os.path.join(output_base, f"vgnir_bi_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": vgnir_bi_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        ...
        plot_confusion_matrix(all_y_true, all_y_pred, title="vgnir_bi Overall Confusion Matrix", class_names=['Non-Urban', 'Urban'])
        confusion_matrix_path = os.path.join(output_base, "overall_confusion_matrix.png")
        plt.savefig(confusion_matrix_path)
        plt.tight_layout()
        plt.show()

    print("vgnir_bi Processing complete.")