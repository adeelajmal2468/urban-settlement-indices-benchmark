...
def calculate_ibi(ndbi, ndvi, mndwi):
    epsilon = 1.0
    return (ndbi - (ndvi + mndwi)) / (ndbi + (ndvi + mndwi)+ epsilon)
...
output_base = r"D:\SeasoNet\Indices Implementation\snow_results\ibi_ndvi"
...
for i in tqdm(range(len(df_all)), desc="Processing samples"):
    ...
    ibi_smooth = gaussian_filter(ibi, sigma=20)
