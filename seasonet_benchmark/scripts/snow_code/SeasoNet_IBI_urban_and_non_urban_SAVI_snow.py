...
def calculate_ibi(ndbi, savi, mndwi):
    epsilon = 1
    return (ndbi - (savi + mndwi)) / (ndbi + (savi + mndwi)+ epsilon)

def create_overlay(ibi, labels, threshold, urban_classes_present, nonurban_classes_present):
    overlay = np.zeros((*ibi.shape, 4))
    urban_mask = ibi > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\snow_results\ibi_savi"
...
# -----------------------------------------------------------------------------
# Process every row in the CSV file for ibi evaluation with a progress bar
# -----------------------------------------------------------------------------
for i in tqdm(range(len(df_all)), desc="Processing samples"):
    ...
    mndwi = calculate_mndwi(band_green, band_swir)

    # Calculate IBI
    ibi = calculate_ibi(ndbi, savi, mndwi)
    ibi_smooth = gaussian_filter(ibi, sigma=19.2)

    valid_ibi = ibi_smooth[np.isfinite(ibi_smooth)]
    if valid_ibi.size == 0:
        print(f"No valid ibi values for {folder_name}.")
        continue
    valid_ibi = valid_ibi.reshape(-1, 1)
    kmeans = KMeans(n_clusters=2, random_state=42).fit(valid_ibi)
    ...
    ibi_classified = (ibi_smooth > biased_threshold).astype(np.uint8)

    structure = np.ones((1, 1))
    ibi_classified = binary_opening(ibi_classified, structure=structure)
    ibi_classified = binary_closing(ibi_classified, structure=structure).astype(np.uint8)
    ibi_classified = remove_small_objects(ibi_classified.astype(bool), min_size=1)
    ibi_classified = remove_small_holes(ibi_classified, area_threshold=1)
    ibi_classified = ibi_classified.astype(np.uint8)
    ibi_classified = median_filter(ibi_classified, size=5)
    ...
    height, width = ibi.shape
    sample_total_pixels = height * width

    # Evaluate metrics only on pixels within the valid mask
    tp_sample = np.sum((ibi_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((ibi_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((ibi_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((ibi_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 50 == 0:
        ...
        plt.subplot(1, 3, 1)
        dbi_plot = plt.imshow(ibi, cmap='gray')
        cbar = plt.colorbar(dbi_plot, label='ibi Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('Original ibi', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(ibi, cmap='gray')
        overlay = create_overlay(ibi, labels, biased_threshold, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plot_path = os.path.join(output_base, f"ibi_sample_{i}.png")
    ...
    all_y_pred.extend(ibi_classified.flatten()[valid_mask.flatten()])
