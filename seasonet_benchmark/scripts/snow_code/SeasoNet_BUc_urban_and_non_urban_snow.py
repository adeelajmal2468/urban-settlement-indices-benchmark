...
def normalize_image(img):
    ...
    norm = (img - img_min) / (img_max - img_min + 1e-6) * 255
    return norm

def calculate_ndvi_cont(nir, red, epsilon=1e-6):
    """Calculate the continuous NDVI image."""
    return (nir - red) / (nir + red + epsilon)

def calculate_ndbi_cont(swir, nir, epsilon=1e-6):
    """Calculate the continuous NDBI image."""
    return (swir - nir) / (swir + nir + epsilon)

def calculate_BUc(ndbi_cont, ndvi_cont):
    """Compute the continuous built-up index (BUc) as the difference."""
    return ndbi_cont - ndvi_cont

def create_overlay(buc, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an overlay image for visualization from the BUc (before thresholding)."""
    overlay = np.zeros((*buc.shape, 4))
    # Here we classify as urban if BUc_norm > threshold
    urban_mask = buc > threshold
...
# Set Paths
# -----------------------------------------------------------------------------
csv_path = r"D:\SeasoNet\filtered_snow_meta_with_both_urban_nonurban_2.csv"
base_path = r"D:\SeasoNet"
output_base = r"D:\SeasoNet\Indices Implementation\snow_results\buc"

# Read the CSV file and use first 100 rows for testing
...
# -----------------------------------------------------------------------------
# Process every row in the CSV file for BUc evaluation with a progress bar
# -----------------------------------------------------------------------------
def process_sample(i_row):
    ...
    # File paths:
    nir_tif_path = os.path.join(folder_path, f"{folder_name}_10m_IR.tif")      # NIR band
    rgb_tif_path = os.path.join(folder_path, f"{folder_name}_10m_RGB.tif")      # Contains Red band (from which we extract red)
    tif_20m_path = os.path.join(folder_path, f"{folder_name}_20m.tif")          # Contains SWIR band (we use band 5)
    label_tif_path = os.path.join(folder_path, f"{folder_name}_labels.tif")
    
    # Load additional mask files (for clouds, snow, etc.)
    ...
    try:
        # Load bands needed for the improved NDBI (BUc) approach
        # For NDVI: need NIR and Red.
        nir, meta_nir = load_and_preprocess_band(nir_tif_path, 1)
        red, _ = load_and_preprocess_band(rgb_tif_path, 3)  # Use red band from RGB
        # For NDBI: need SWIR and NIR. Resample SWIR band (assumed to be band 5 in tif_20m)
    ...
    # Compute continuous built-up image BUc = NDBIc - NDVIc
    buc = calculate_BUc(ndbi_cont, ndvi_cont)
    
    # Normalize BUc to 0-255
    buc_norm = normalize_image(buc)
    
    # According to the paper, a threshold of around 102 (in the normalized range) works well.
    threshold_value = 102
    buc_classified = (buc_norm > threshold_value).astype(np.uint8)
    
    # Apply morphological operations to smooth the classification
    structure = np.ones((1,1))
    buc_classified = binary_opening(buc_classified, structure=structure)
    buc_classified = binary_closing(buc_classified, structure=structure).astype(np.uint8)
    buc_classified = remove_small_objects(buc_classified.astype(bool), min_size=500)
    buc_classified = remove_small_holes(buc_classified, area_threshold=500)
    buc_classified = buc_classified.astype(np.uint8)
    buc_classified = median_filter(buc_classified, size=1)
    ...
    height, width = buc.shape
    total_pixels_sample = height * width
    
    # Evaluate metrics only on pixels within the valid mask
    tp_sample = np.sum((buc_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((buc_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((buc_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((buc_classified == 0) & (urban_mask == 0) & valid_mask)
    sample_iou = tp_sample / (tp_sample + fp_sample + fn_sample) if (tp_sample + fp_sample + fn_sample) > 0 else 0
    if sample_iou < 0.6:
        return None

    # Save sample images every iteration (adjust as needed)
    plt.figure(figsize=(24, 8))
    plt.subplot(1, 3, 1)
    plt.imshow(buc_norm, cmap='gray')
    cbar = plt.colorbar(label='BUc_norm Value', fraction=0.046, pad=0.04)
    plt.title('Normalized BUc', fontsize=14, pad=20)
    plt.axis('off')
    
    plt.subplot(1, 3, 2)
    plt.imshow(buc_norm, cmap='gray')
    overlay = create_overlay(buc_norm, labels, threshold_value, urban_classes_present, nonurban_classes_present)
    plt.imshow(overlay)
    ...
    plt.title('BUc Overlay', fontsize=14, pad=20)
    plt.axis('off')
    ...
    plot_path = os.path.join(output_base, f"BUc_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": buc_classified.flatten()[valid_mask.flatten()]
    }
