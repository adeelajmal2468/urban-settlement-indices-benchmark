...
def calculate_mbsi(red, green):
    epsilon = 1e-6
    numerator = ((red - green) * 2)
    denominator = (((red + green) - 2) + epsilon)
    return numerator / denominator

def calculate_osavi(nir, red):
    epsilon = 1e-6
    numerator = (nir - red)
    denominator = (nir + red + 0.16 + epsilon)
    return numerator / denominator

def calculate_cbci(mbsi, osavi):
    A = 0.51
    return (A + 1) * mbsi - osavi + A

def create_overlay(cbci, labels, threshold, urban_classes_present, nonurban_classes_present):
    overlay = np.zeros((*cbci.shape, 4))
    urban_mask = cbci > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\spring_results\cbci"
...
def process_sample(i_row):
    """
    Process one sample: load bands and masks, calculate cbci, perform segmentation,
    ...
    mbsi = calculate_mbsi(band_red, band_green)
    osavi = calculate_osavi(band_nir, band_red)

    # Calculate cbci
    cbci = calculate_cbci(mbsi, osavi)
    cbci_smooth = gaussian_filter(cbci, sigma=3)

    valid_cbci = cbci_smooth[np.isfinite(cbci_smooth)]
    if valid_cbci.size == 0:
        print(f"No valid cbci values for {folder_name}.")
        return None
    valid_cbci = valid_cbci.reshape(-1, 1)
    kmeans = KMeans(n_clusters=2, random_state=42).fit(valid_cbci)
    ...
    cbci_classified = (cbci_smooth > biased_threshold).astype(np.uint8)
    structure = np.ones((1, 1))
    cbci_classified = binary_opening(cbci_classified, structure=structure)
    cbci_classified = binary_closing(cbci_classified, structure=structure).astype(np.uint8)
    cbci_classified = remove_small_objects(cbci_classified.astype(bool), min_size=100)
    cbci_classified = remove_small_holes(cbci_classified, area_threshold=100)
    cbci_classified = cbci_classified.astype(np.uint8)
    cbci_classified = median_filter(cbci_classified, size=3)
    ...
    tp_sample = np.sum((cbci_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((cbci_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((cbci_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((cbci_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 90 == 0:
        ...
        plt.subplot(1, 3, 1)
        dbi_plot = plt.imshow(cbci, cmap='gray')
        cbar = plt.colorbar(dbi_plot, label='cbci Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('Original cbci', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(cbci, cmap='gray')
        overlay = create_overlay(cbci, labels, biased_threshold, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plot_path = os.path.join(output_base, f"cbci_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": cbci_classified.flatten()[valid_mask.flatten()]
    }
...
if __name__ == '__main__':
    ...
    with ProcessPoolExecutor(max_workers=8) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Processing samples"):
            ...
            # Store per-sample arrays (not flattened into huge Python lists)
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        # Concatenate per-sample arrays once to avoid giant Python lists of ints
        y_true_overall = np.concatenate(all_y_true)
        y_pred_overall = np.concatenate(all_y_pred)
