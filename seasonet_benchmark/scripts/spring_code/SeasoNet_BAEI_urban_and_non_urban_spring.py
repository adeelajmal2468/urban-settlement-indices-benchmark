...
def calculate_baei(red, green, swir):
    epsilon = 1e-6
    L = 0.3
    return (red + L) / (green + swir + epsilon)

def create_overlay(baei, labels, threshold, urban_classes_present, nonurban_classes_present):
    overlay = np.zeros((*baei.shape, 4))
    urban_mask = baei > threshold
...
output_base = r"D:\SeasoNet\Indices Implementation\spring_results\baei"
...
def process_sample(i_row):
    """
    Process one sample: load bands and masks, calculate BAEI, perform segmentation,
    ...
    # Calculate BAEI
    baei = calculate_baei(band_red, band_green, band_swir)
    baei_smooth = gaussian_filter(baei, sigma=5)

    valid_baei = baei_smooth[np.isfinite(baei_smooth)]
    if valid_baei.size == 0:
        print(f"No valid baei values for {folder_name}.")
        return None
    valid_baei = valid_baei.reshape(-1, 1)
    kmeans = KMeans(n_clusters=2, random_state=42).fit(valid_baei)
    ...
    baei_classified = (baei_smooth > biased_threshold).astype(np.uint8)
    structure = np.ones((1, 1))
    baei_classified = binary_opening(baei_classified, structure=structure)
    baei_classified = binary_closing(baei_classified, structure=structure).astype(np.uint8)
    baei_classified = remove_small_objects(baei_classified.astype(bool), min_size=100)
    baei_classified = remove_small_holes(baei_classified, area_threshold=100)
    baei_classified = baei_classified.astype(np.uint8)
    baei_classified = median_filter(baei_classified, size=5)
    ...
    tp_sample = np.sum((baei_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((baei_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((baei_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((baei_classified == 0) & (urban_mask == 0) & valid_mask)
    ...
    if i % 90 == 0:
        ...
        plt.subplot(1, 3, 1)
        dbi_plot = plt.imshow(baei, cmap='gray')
        cbar = plt.colorbar(dbi_plot, label='baei Value', fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title('Original baei', fontsize=14, pad=20)
        plt.axis('off')

        plt.subplot(1, 3, 2)
        plt.imshow(baei, cmap='gray')
        overlay = create_overlay(baei, labels, biased_threshold, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plot_path = os.path.join(output_base, f"baei_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": baei_classified.flatten()[valid_mask.flatten()]
    }
