...
def find_otsu_threshold(SwiRed_values):
    """Find threshold using Otsu's method with guards for tiny ranges."""
    finite_vals = SwiRed_values[np.isfinite(SwiRed_values)]
    ...
    # avoid histogram errors when data range is almost zero
    if vmax - vmin <= 1e-6:
        return vmin
    ...
    try:
        ...
        # fallback in case skimage still complains
        threshold = 0.5 * (vmin + vmax)

    return float(threshold)


def compute_SwiRed(swir1, red):
    eps = 1e-6
    numerator = swir1 - red
    denominator = swir1 + red + eps
    SwiRed = numerator / denominator
    return SwiRed


def create_overlay(SwiRed, labels, threshold, urban_classes_present, nonurban_classes_present):
    """Create an RGBA overlay from the thresholded SwiRed."""
    overlay = np.zeros((*SwiRed.shape, 4))
    urban_mask = SwiRed > threshold
...
def plot_confusion_matrix(y_true, y_pred, title, class_names=None, ax=None):
    ...
    # force 0 and 1 as labels and consistent 2x2 matrix
    ...
    cm_percent = cm.astype(float) / row_sums * 100.0
...
output_base = r"D:\SeasoNet\Indices Implementation\spring_results\swired"
...
def process_sample(i_row):
    ...
    # Compute the SwiRed index
    SwiRed = compute_SwiRed(band_swir_1, band_red)

    # Smooth
    SwiRed_smooth = gaussian_filter(SwiRed, sigma=60)

    valid_SwiRed = SwiRed_smooth[np.isfinite(SwiRed_smooth) & valid_mask]
    if valid_SwiRed.size < 10:
        return None

    # Use Otsu's method for threshold selection with guards
    T_otsu = find_otsu_threshold(valid_SwiRed)
    if not np.isfinite(T_otsu):
        return None

    SwiRed_classified = (SwiRed_smooth > T_otsu).astype(np.uint8)

    # Fine-tuned morphological operations
    structure = np.ones((1, 1))
    SwiRed_classified = binary_opening(SwiRed_classified, structure=structure).astype(np.uint8)
    SwiRed_classified = binary_closing(SwiRed_classified, structure=structure).astype(np.uint8)
    SwiRed_classified = remove_small_objects(SwiRed_classified.astype(bool), min_size=100)
    SwiRed_classified = remove_small_holes(SwiRed_classified, area_threshold=100)
    SwiRed_classified = median_filter(SwiRed_classified.astype(np.uint8), size=1)
    ...
    tp_sample = np.sum((SwiRed_classified == 1) & (urban_mask == 1) & valid_mask)
    fp_sample = np.sum((SwiRed_classified == 1) & (urban_mask == 0) & valid_mask)
    fn_sample = np.sum((SwiRed_classified == 0) & (urban_mask == 1) & valid_mask)
    tn_sample = np.sum((SwiRed_classified == 0) & (urban_mask == 0) & valid_mask)

    denom = tp_sample + fp_sample + fn_sample
    sample_iou = tp_sample / denom if denom > 0 else 0.0
    if sample_iou < 0.6:
        return None

    if i % 90 == 0:
        SwiRed_min = np.nanmin(SwiRed)
        SwiRed_max = np.nanmax(SwiRed)

        plt.figure(figsize=(24, 8))

        plt.subplot(1, 3, 1)
        plt.imshow(SwiRed, cmap="gray", vmin=SwiRed_min, vmax=SwiRed_max)
        cbar = plt.colorbar(label="SwiRed Value", fraction=0.046, pad=0.04)
        cbar.ax.tick_params(labelsize=10)
        plt.title("SwiRed", fontsize=14, pad=20)
        plt.axis("off")

        plt.subplot(1, 3, 2)
        plt.imshow(SwiRed, cmap="gray", vmin=SwiRed_min, vmax=SwiRed_max)
        overlay = create_overlay(SwiRed, labels, T_otsu, urban_classes_present, nonurban_classes_present)
        plt.imshow(overlay)
        ...
        plt.title("SwiRed Classification Overlay", fontsize=14, pad=20)
        plt.axis("off")
        ...
        plot_path = os.path.join(output_base, f"SwiRed_sample_{i}.png")
    ...
    return {
        ...
        "y_pred": SwiRed_classified.flatten()[valid_mask.flatten()],
    }
...
if __name__ == "__main__":
    ...
    if total_pixels_overall > 0:
        ...
        overall_iou = overall_tp / (overall_tp + overall_fp + overall_fn) if (overall_tp + overall_fp + overall_fn) else 0.0
    else:
        overall_accuracy = overall_precision = overall_recall = overall_f1 = overall_iou = 0.0
    ...
    if len(all_y_true) > 0 and len(all_y_pred) > 0:
        y_true = np.asarray(all_y_true, dtype=np.uint8)
        y_pred = np.asarray(all_y_pred, dtype=np.uint8)

        plt.figure(figsize=(8, 6))
        plot_confusion_matrix(
            y_true,
            y_pred,
            title="SwiRed Overall Confusion Matrix",
    ...
    print("SwiRed Processing complete.")
