...
# ======= 2. Indices =======
mndsi = (b7 - b8) / (b7 + b8 + 1e-6)
ndvi  = (b5 - b4) / (b5 + b4 + 1e-6)
TC1 = (0.3029*b2 + 0.2786*b3 + 0.4733*b4 + 0.5599*b5 + 0.5080*b6 + 0.1872*b7)
TC2 = (-0.2941*b2 - 0.2430*b3 - 0.5424*b4 + 0.7276*b5 + 0.0713*b6 - 0.1608*b7)
TC3 = (0.1511*b2 + 0.1973*b3 + 0.3283*b4 + 0.3407*b5 - 0.7117*b6 - 0.4559*b7)
norm = lambda a: (a - np.nanmin(a)) / (np.nanmax(a) - np.nanmin(a) + 1e-6)
H, V, L = norm(TC1), norm(TC2), norm(TC3)
bci  = ((H + L)/2 - V) / ((H + L)/2 + V + 1e-6)
rui  = bci / (mndsi + 1e-6)
nrui = (rui - mndsi) / (rui + mndsi + 1e-6)
for arr in (mndsi, ndvi, bci, rui, nrui): arr[~np.isfinite(arr)] = np.nan

indices = {"MNDSI": mndsi, "BCI": bci, "RUI": rui, "NRUI": nrui}
...
URBAN_COLOR = np.array([1.0, 0.1, 0.1])      # bright red
NON_URBAN_COLOR = np.array([0.65, 0.65, 0.65]) # light gray
...
for idx_name, idx in indices.items():
    ...
    # --- Panel 2: Dynamic World classes with legend ---
    ...
    # --- Panel 3: Urban/Non-Urban mask overlays on grayscale index image ---
    idx_disp = np.copy(idx)
    idx_disp[~np.isfinite(idx_disp)] = np.nan
    axs[2].imshow(idx_disp, cmap='gray')
    
    # Overlay: GT mask (cyan), prediction (red), all others (gray)
...
header = "{:<6} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>6} {:>8}".format(
         "Index","IoU","F1","Pr","Re","Acc","BAc","Kappa","AUC","TP","NDVI_thr")
print(header)
print("-"*len(header))
for k,v in best_results.items():
    print("{:<6} {:8.3f} {:8.3f} {:8.3f} {:8.3f} {:8.3f} {:8.3f} {:8.3f} {:8.3f} {:6.0f} {:8.2f}".format(
...
print("Predicted Urban Fraction (Best Index): {:.2%}".format(pred.mean()))
import collections
import numpy as np

# Assuming label_viz is the resized Dynamic World labels (int, 0–8)
