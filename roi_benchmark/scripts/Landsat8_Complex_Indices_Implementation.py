...
landsat_annual_path = r'C:\Users\Adeel Ajmal\Downloads\L8_BahriaPhase7_AnnualMax11Bands_20230101_20250701.tif'
...
outdir       = "urban_index_eval_complex_all"
save_figs    = True
...
with rasterio.open(landsat_annual_path) as src:
    stack = src.read().astype('float32')
b1_annual,b2_annual,b3_annual,b4_annual,b5_annual,b6_annual,b7_annual,b8_annual,b9_annual,b10_annual,b11_annual = stack
...
# ---- LST (Kelvin, Sun et al. Eq. 56) ----
λ = 10.8e-6        # central wavelength (m) for L8 band 10
ρ_const = 1.438e-2 # m·K
Tb = b10           # Band 10: TOA Brightness Temp in Kelvin
LST = Tb / (1 + (λ * Tb / ρ_const) * np.log(ε))
LST_norm = (LST - np.nanmin(LST)) / (np.nanmax(LST) - np.nanmin(LST) + 1e-6)

# --- MNDSI_Liu ---
rho_swir1_norm = (rho_swir1 - np.nanmin(rho_swir1)) / (np.nanmax(rho_swir1) - np.nanmin(rho_swir1) + 1e-6)
num = LST + NTL_norm - (MNDWI + rho_swir1_norm)
den = LST + NTL_norm + MNDWI + rho_swir1_norm + 1e-6
MNDSI_Liu = num / den
indices['MNDSI_Liu'] = MNDSI_Liu

# --- VTLI ---

Pv = ((NDVI_max - 0.2) / 0.3)**2
ε = np.where(
        NDVI_max < 0.2,
        0.979 - 0.035 * b4_annual,
        np.where(
            NDVI_max <= 0.5,
            0.986 + 0.004 * Pv,
            0.990
...
λ = 10.8e-6        # central wavelength (m) for L8 band 10
ρ_const = 1.438e-2 # m·K
Tb = b10_annual           # Band 10: TOA Brightness Temp in Kelvin
LST_max = Tb / (1 + (λ * Tb / ρ_const) * np.log(ε))

VTLI = (1 - NDVI_max) * LST_max * NTL_norm
indices['VTLI'] = VTLI

# --- TVANUI ---
epsilon = 1e-6
LST_div = LST_norm / (NDVI_max + epsilon)
arctan_part = np.arctan(LST_div) / (np.pi / 2)
NTL_063 = NTL_norm
TVANUI = arctan_part * NTL_063
indices['TVANUI'] = TVANUI

# --- LST itself (optional visualization) ---
indices['LST'] = LST
indices['LST_max'] = LST_max
